...
class MetadataSearch:
    ...
    def get_undescribed_files(self, file_paths: list[str]) -> list[str] | None:
        ...
        VFS-safe: cache lookup uses stat-only keys (no content read).
        ...
        for fp in file_paths:
            ...
            cache_key = self.make_description_cache_key(fp)
    ...
    def _get_auto_description(self, file_path: str, generate_desc_if_not_in_cache: bool = True) -> str:
        ...
        # Extract the extension to obtain proper description method
        ext = os.path.splitext(file_path)[1].lower()
        method_name = self._extension_to_describe_method(ext)
        if method_name is None:
    ...
    def generate_full_description(self, file_path: str, media_folder: str, generate_desc_if_not_in_cache: bool = True) -> str:
        """Build the full metadata description for a file.

        Local files: filename + path + embedding proxy + auto-desc + internal metadata + {filename}.meta.
        Remote files: filename + path + {filename}.meta only — never touch the original file.
        ...
        Args:
            ...
            media_folder: Media directory root (used for relative path display).
            generate_desc_if_not_in_cache: If False, skips OmniDescriptor auto-description
                generation for files not already in cache (fast mode for training).
        """
        relative_path = file_path
        is_local = vfs.is_local_url(file_path)
        ...
        if is_local:
            ...
            auto_desc = self._get_auto_description(file_path, generate_desc_if_not_in_cache=generate_desc_if_not_in_cache)
            if auto_desc:
    ...
    def process_files(self, file_paths: list[str], callback=None, media_folder: str = None, 
        ...
        for ind, file_path in enumerate(file_paths):
            ...
            embedding_list = self._process_single_file_meta(file_path, media_folder, 
                generate_embs_if_not_in_cache = generate_embs_if_not_in_cache)
